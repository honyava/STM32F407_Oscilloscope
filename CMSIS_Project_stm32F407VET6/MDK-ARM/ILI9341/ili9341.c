...
uint16_t find_max(uint16_t *m, uint16_t size)
{
	uint16_t max = 0;
...
{
	lcdDrawVLine(x, y, y + side_y, color); //vertical line with step 25 pixels
	lcdDrawVLine(x + side_x, y, y + side_y, color); //vertical line with step 25 pixels
	lcdDrawHLine(x, x + side_x, y, color); //horizontal line with step 25 pixels
	lcdDrawHLine(x, x + side_x, y + side_y, color); //horizontal line with step 25 pixels
}
void lcdGrid(uint16_t x, uint16_t y, uint16_t color)
{
	...
	for (uint8_t i = 0; i < 8; i++)
	{
		for (uint8_t j = 0; j < 8; j++)
		{
			...
			if (count < 56)
			{
				lcdDrawVLine(x + 5*count,118, 122, color);
				if (count < 40) lcdDrawHLine(158, 162, 20 + 5*count, color);
			}
			count++;
		}
	}
